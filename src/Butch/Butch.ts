...
export class Program extends Block {
  ...
  protected logicsBody(env: Environment): Value {
    ...
    try {
      ...
      RuntimeError.throwIdentifierError(
        this,
        "Symbol 'main' must be defined as a function",
      );
    }
...
export class ButchBuilder {
  ...
  private streams: { id: string; write: (arg: string) => void }[] = [];
  ...
  constructor(codes: { [key: string]: string }) {
    ...
    this.builders = new Map<string, Builder>([
      ...
      [
        ...
        info =>
          new PrintBlock(
            ...
            this.streams.map(stream => stream.write),
          ),
      ],
      [
        this.c.set,
      ...
      [
        this.c.container,
        info => new ContainerBlock(info.obj.extension.builtContent),
      ],
    ]);
  ...
  saveButchCodes(newName: string | undefined = undefined) {
    ...
    const pathToCodesSet =
      rnfs.DocumentDirectoryPath + `/.bch/${newName ?? "codes"}.set.txt`;
    ...
    for (let i = 0; i < content.length; ++i) {
      ...
      switch (content[i][this.c.type]) {
        case this.c.function:
          prog.useFunction(
            String(content[i][this.c.name]),
            this.buildFunction(info),
          );
          break;
